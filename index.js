...
const PSIKA_G11_GUILD_ID = '1430923289160847500';
...
client.once('ready', () => {
  ...
  ensureThreadsJoinedForGuild(PSIKA_G11_GUILD_ID);
...
client.on('messageCreate', async (message) => {
  ...
  const isPracticas = (
    guildId === '1351968535580114984' ||
    guildId === LECTOR_AKAE_GUILD_ID ||
    guildId === PSIKA_G11_GUILD_ID
  ) && isPracticasCategory(canal, config, message.guild);
  ...
  if (guildId === '1411656675408220211') {
    ...
    if (!shouldTriggerAnalisisWebhook) {
      console.log('[DEBUG_BHIMBIRA] Condiciones no cumplidas', {
        ...
        threadParentCategoryId,
        candidateChannelIds, candidateCategoryIds,
        tipo_canal, embajador
      });
    }
  }
  ...
  if (guildId === LECTOR_AKAE_GUILD_ID) {
    ...
    if (!shouldTriggerAnalisisWebhook) {
      console.log('[DEBUG_LECTOR] ❌ NO se cumplió ninguna condición para análisis', {
        canalId, categoriaId, threadParentCategoryId,
        candidateChannelIds, candidateCategoryIds,
        tipo_canal, embajador,
  ...
  // ========================================
  // REGLAS PARA PSIKA® G11
  // ========================================
  if (guildId === PSIKA_G11_GUILD_ID) {
    console.log('[DEBUG_PSIKA_G11] Mensaje recibido', {
      canal: `${canal.name} (${canalId})`,
      canalType: canal.type,
      categoriaId,
      threadParentChannel: threadParentChannel ? `${threadParentChannel.name} (${threadParentChannel.id})` : 'none',
    ...
    if (Object.values(config.canalesFijos || {}).some(id => candidateChannelIds.includes(id))) {
      shouldTriggerAnalisisWebhook = true;
      console.log('[DEBUG_PSIKA_G11] ✅ Match en canalesFijos');
    }
    ...
    if (!shouldTriggerAnalisisWebhook && config.categoriasPorEmbajador) {
      for (const [nombreEmb, embajadorData] of Object.entries(config.categoriasPorEmbajador)) {
        ...
        if (categoriasPermitidas.some(id => candidateCategoryIds.includes(id))) {
          shouldTriggerAnalisisWebhook = true;
          console.log(`[DEBUG_PSIKA_G11] ✅ Match en categoría embajador/urgencias: ${nombreEmb}`);
          break;
        }
        const canalesEmbajador = [
          embajadorData.soytuembajador,
        ...
        if (canalesEmbajador.some(id => candidateChannelIds.includes(id))) {
          shouldTriggerAnalisisWebhook = true;
          console.log(`[DEBUG_PSIKA_G11] ✅ Match en canal específico embajador: ${nombreEmb}`);
          break;
        }
      }
    }
    ...
    if (!shouldTriggerAnalisisWebhook) {
      console.log('[DEBUG_PSIKA_G11] ❌ NO se cumplió ninguna condición para análisis', {
        canalId, categoriaId, threadParentCategoryId,
        candidateChannelIds, candidateCategoryIds,
        tipo_canal, embajador
      });
  ...
  if (
    ...
    (guildId === PSIKA_G11_GUILD_ID && config.canalesFijos?.soporte && canalId === config.canalesFijos.soporte) ||
    isSupportChannel(canal, config)
  ) {
    try {
      console.log(`[DEBUG] Enviando a webhookFAQ: ${config.webhookFAQ}`);
      const faqPayload = { ...payload, question: message.content };
...
client.on('threadCreate', (thread) => {
  ...
  if (!guildId) return;
  if (
    guildId === BHIMBIRA_GUILD_ID ||
    guildId === LECTOR_AKAE_GUILD_ID ||
    guildId === PSIKA_G11_GUILD_ID ||
    threadBelongsToPracticas(thread)
  ) {
...
client.on('threadUpdate', (_oldThread, newThread) => {
  ...
  if (
    ...
    guildId === LECTOR_AKAE_GUILD_ID ||
    guildId === PSIKA_G11_GUILD_ID ||
    threadBelongsToPracticas(newThread)
  ) {
